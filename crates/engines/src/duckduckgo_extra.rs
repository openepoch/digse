//! DuckDuckGo Extra (general web) engine implementation
//!
//! This engine drives
//! the images/videos/news categories by hitting DDG's `i.js`/`v.js`/`news.js`
//! endpoints. In this port we implement the general web fallback variant that
//! scrapes the DDG HTML endpoint (the same approach as the existing
//! `duckduckgo.rs` engine), so the engine can return general web results even
//! without a VQD token.
...
/// DuckDuckGo extra (general web) engine
pub struct DuckDuckGoExtraEngine {
    metadata: EngineMetadata,
    client: reqwest::Client,
}

impl DuckDuckGoExtraEngine {
    pub fn new() -> Self {
        let metadata = EngineMetadata {
            ...
            requires_auth: false,
            timeout_seconds: 10,
            description: "DuckDuckGo extra (web).".to_string(),
        ...
        let client = reqwest::Client::builder()
            ...
            .build()
            .expect("Failed to create DDG extra HTTP client");

        DuckDuckGoExtraEngine { metadata, client }
    }

    fn parse_html(&self, html: &str) -> Vec<DdgExtraResult> {
        ...
        for sel_str in selectors.iter() {
            ...
            for element in document.select(&sel) {
                if let Some(a) = element.select(&title_sel).next() {
                    ...
                    // DDG wraps hrefs in a redirect: strip if present
...
#[async_trait]
impl Engine for DuckDuckGoExtraEngine {
    fn name(&self) -> &str {
        &self.metadata.name
    }
